...
class GameManager {
  ...
  spawnEnemies (enemyToSpawn, spawnTimeMax, spawnTimeMin, spawnPosXMax, spawnPosXMin, canvas, c) {
    if (!spawnCooldown) {
      ...
      spawnCooldown = true
      setTimeout(function () {
      enemyToSpawn.push(new Enemy(spawnPosX, 0, canvas, c))
      spawnCooldown = false
    }, spawnTime)
    }   
  }
  ...
  ///param<bulets, positionX, positionY, velocityY///
    ...
    bulletUpdator (canvas, c, enemyBullets) {
      let bulletsToRemove = [];
